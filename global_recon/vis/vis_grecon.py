...
class GReconVisualizer(Visualizer3D):
        ...
        if self.coord in {'cam', 'cam_in_world'}:
            ...
            if self.use_y_up_coord:
                ...
                if self.render_cam_pos is not None:
                    ...
                    # self.cam_pos_array.append(self.pl.camera.position)
                    # zyx
                    # self.cam_pos_array.append([self.pl.camera.position[2],self.pl.camera.position[1],self.pl.camera.position[0]])
                    
                    # print('cam_pos_array:', self.cam_pos_array)
    ...
    def update_camera(self, interactive):
        # print("from vis_gecon.py: update_camera(self, interactive)") #yes
        ...
        if self.coord == 'cam_in_world':
            ...
            # print("from vis_grecon.py: ",type(self.pl.camera.position))
            # print("from vis_grecon.py: ",len(self.pl.camera.position))
            # print("from vis_grecon.py: ",self.pl.camera.position)
            # print("from vis_grecon.py: ",self.pl.camera.focal_point)

            # self.cam_pos_array.append(self.pl.camera.position)
            self.cam_pos_array.append([self.pl.camera.position[2],self.pl.camera.position[1],self.pl.camera.position[0]])
            # print('cam_pos_array:', self.cam_pos_array)
            # import pickle
            # with open("/mnt/datadrive/annh/GLAMR/global_recon/cam_position.pkl","wb") as f:
            #     pickle.dump(self.cam_pos_array, f)
    ...
    def update_scene(self):
        # print("from vis_gecon.py: update_scene(self)") #yes
        super().update_scene()
        if self.verbose:
            print(self.fr)
