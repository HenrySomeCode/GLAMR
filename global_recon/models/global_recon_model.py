...
class GlobalReconOptimizer:
    ...
    def init_data(self, in_dict):
        # print("from global_recon_model.py: init_data") #yes
        person_data = dict()
        # print("from global_recon_model.py: ", in_dict.keys()) #dict_keys(['est', 'gt', 'gt_meta', 'seq_name']
        
        # print("from global_recon_model.py: ", in_dict['est'].keys())
        #dict_keys(['pred_uvd', 'pred_xyz_17', 'pred_xyz_29', 'pred_xyz_24_struct', 'pred_scores', 'pred_camera', 
        # 'pred_betas', 'pred_thetas', 'pred_phi', 'pred_cam_root', 'transl', 'transl_camsys', 'bbox', 'height', 
        # 'width', 'img_path']

        # need to find smpl_pose_quat_wroot???*

        # print("from global_recon_model.py: ", in_dict['est'][0]) #error
        
        # print("from global_recon_model.py: ", in_dict['est']['bbox'])
        # [[228.23864746 227.53848267 736.64099121 735.94082642]
        # [227.97019958 227.62748718 736.25160217 735.90888977]
        # [228.2961731  228.28204346 735.23471069 735.22058105]
        # ...
        # [356.28210449 189.89135742 916.01916504 749.62841797]
        # [367.83370972 194.25003052 923.84170532 750.25802612]
        # [374.67666626 198.42538452 927.13754272 750.88626099]]

        # print("from global_recon_model.py: ", len(in_dict['est']['bbox']))
        
        num_fr = len(in_dict['est'][0]['bboxes_dict']['exist'])
        # num_fr = len(in_dict['est']['bbox'])
        ...
        # print("from global_recon_model.py: src_joint_info: ", src_joint_info)
        dst_joint_info = get_joints_info("body26fk")
        # print("from global_recon_model.py: dst_joint_info: ", dst_joint_info)

        dst_dict = dict((v, k) for k, v in dst_joint_info.name.items())
        # print("from global_recon_model.py: dst_dict: ", dst_dict)
        # {'right_ankle': 8, 'right_knee': 5, 'right_hip': 2, 'right_shoulder': 21, 'right_elbow': 23, 
        # 'right_wrist': 25, 'left_ankle': 7, 'left_knee': 4, 'left_hip': 1, 'left_shoulder': 20, 'left_elbow': 22, 
        # 'left_wrist': 24, 'neck': 6, 'pelvis': 0, 'torso': 3, 'nose': 15, 'right_eye': 17, 'right_ear': 19, 
        # 'left_eye': 16, 'left_ear': 18, 'right_heel': 14, 'right_big_toe': 10, 'right_small_toe': 12, 'left_heel': 13, 
        # 'left_big_toe': 9, 'left_small_toe': 11}
        
        smpl_to_body26fk = np.array([(dst_dict[v], k) for k, v in src_joint_info.name.items() if v in dst_dict.keys()])
        print("from global_recon_model.py: smpl_to_body26fk.shape: ", smpl_to_body26fk.shape) #(14,2)
        
        if self.est_type == 'hybrik':
            for idx, pose_dict in in_dict['est'].items():
                # pose_dict = in_dict['est']
                # print("from global_recon_model.py: idx: ", idx)
                # print("from global_recon_model.py: pose_dict: ", pose_dict)
                new_dict = dict()
                new_dict['visible'] = visible = pose_dict['bboxes_dict']['exist'].copy() # represent of exist bboxes
                # print("from global_recon_model.py: new_dict['visible'].shape: ", new_dict['visible']) #(184,1) => [1.,1.,...]
                # new_dict['visible'] = visible = pose_dict['bbox'].copy()
                ...
                # print("from global_recon_model.py: new_dict['exist_len']", new_dict['exist_len'])#184
                new_dict['max_len'] = max_len = visible.shape[0]
                # print("from global_recon_model.py: new_dict['max_len']", new_dict['max_len']) #184
                new_dict['frames'] = np.arange(max_len)
                # print("from global_recon_model.py: new_dict['frames']: ", new_dict['frames']) # [0,1,2...183] 
                ...
                # print("from global_recon_model.py: new_dict['frame2ind']", new_dict['frame2ind']) #[0:0, 1:1,...183:183]
                new_dict['scale'] = None
                
                smpl_pose_wroot = quaternion_to_angle_axis(torch.tensor(pose_dict[f'smpl_pose_quat_wroot'], device=self.device)).cpu().numpy()

                # smpl_pose_wroot = quaternion_to_angle_axis(torch.tensor(pose_dict[f'pred_cam_root'], device=self.device)).cpu().numpy()
                # print("from global_recon_model.py: ", smpl_pose_wroot)
                print("from global_recon_model.py: ", smpl_pose_wroot.shape) # (330, 54, 3)
                # print("from global_recon_model.py: ", smpl_pose_wroot[:, 1:].shape)
                # new_dict['smpl_pose'] = smpl_pose_wroot[:, 1:].reshape(-1, 69)
                # new_dict['smpl_pose'] = smpl_pose_wroot[:, 1:].reshape(-1, 53)
                ...
                # new_dict['smpl_pose_gt'] = in_dict['pred_thetas'][:, 3:]
                # new_dict['smpl_beta'] = pose_dict['pred_betas']
                ...
                # new_dict['root_trans_cam'] = pose_dict['transl_camsys']
                
                smpl_joints2d = pose_dict['kp_2d'][:, :24]
                # smpl_joints2d = pose_dict['pred_xyz_24_struct'][:, :24]
                ...
                # new_dict['cam_K'] = pose_dict['pred_camera'].astype(np.float32)
        ...
        # print("from global_recon_model.py: data", data['person_data'][0].keys())
        #'visible', 'visible_orig', 'fr_start', 'fr_end', 'exist_frames', 'exist_len', 'max_len', 'frames', 'vis_frames', 
        # 'invis_frames', 'frame2ind', 'scale', 'smpl_pose', 'smpl_beta', 'smpl_orient_cam', 'root_trans_cam', 
        # 'kp_2d', 'kp_2d_score', 'kp_2d_aligned', 'cam_K', 'root_trans_world', 'smpl_orient_world', 'root_trans_world_base', 
        # 'smpl_orient_world_base', 'smpl_pose_nofill', 'infilled', 'traj_predicted', 'traj_local_pred', 'person_transform_world', 
        # 'person_transform_cam', 'person2cam', 'smpl_orient_world_res', 'root_trans_world_res', 'traj_local_xy', 'traj_local_dxy', 
        # 'traj_local_heading', 'traj_local_dheading', 'traj_local_z', 'traj_local_rot'
    ...
    def optimize_main(self, data, opt_variables, opt_lr, opt_niters, loss_cfg, opt_meta):
        ...
        def closure():
            ...
            # print("from gloabl_recon_model.py: loss_cfg: ",loss_cfg)
            #{'rel_transform': {'trans_weight': 0.0, 'weight': 200}, 'kp_2d': {'weight': 1.0, 'min_conf': 0.3}, 
            # 'kp_2d_dist': {'weight': 1.0, 'min_conf': 0.3, 'monitor_only': True}, 'cam_traj_rot': {'rot_type': '6d', 'weight': 100000.0}, 
            # 'traj_rot_smoothness': {'weight': 1000.0}, 'local_traj_dxy_reg': {'weight': 300.0}, 
            # 'local_traj_dheading_reg_new': {'weight': 3000.0}, 'local_traj_rot_reg': {'weight': 5000.0}, 
            # 'local_traj_z_reg': {'weight': 100.0}, 'cam_inv_trans_residual_reg': {'weight': 100.0}, 
            # 'cam_inv_rot_smoothness': {'weight': 10.0}, 'cam_origin_smoothness': {'weight': 1000.0}, 'cam_up_reg': {'weight': 1000000.0}
    ...
    def optimize(self, in_dict, continue_opt=False):
        # print("from global_recon_model.py: in_dict.keys()", in_dict.keys()) #  dict_keys(['est', 'gt', 'gt_meta', 'seq_name'])
        # 'smpl_pose_quat_wroot', 'smpl_beta', 'root_trans', 'kp_2d', 'cam_K', 'frames', 'frame2ind', 'bboxes_dict'
        data = tensor_to(in_dict, self.device) if continue_opt else self.init_data(in_dict)
        # print("from global_recon_model.py: data.keys()", data.keys()) 
        # dict_keys(['seq_name', 'person_data', 'seq_len', 'fr_num_persons', 'cam_pose', 'cam_pose_inv', 'cam_inv_rot_residual', 
        # 'cam_inv_trans_residual', 'rel_transform_cam', 'gt', 'gt_meta', 'meta', 'pose_infer_cam_pose_inv'])

        print("from global_recon_model.py: data['person_data'][0].keys()", data['person_data'][0].keys()) 
        # dict_keys(['visible', 'visible_orig', 'fr_start', 'fr_end', 'exist_frames', 'exist_len', 'max_len', 'frames', 
        # 'vis_frames', 'invis_frames', 'frame2ind', 'scale', 'smpl_pose', 'smpl_beta', 'smpl_orient_cam', 'root_trans_cam', 
        # 'kp_2d', 'kp_2d_score', 'kp_2d_aligned', 'cam_K', 'root_trans_world', 'smpl_orient_world', 'root_trans_world_base', 
        # 'smpl_orient_world_base', 'smpl_pose_nofill', 'infilled', 'traj_predicted', 'traj_local_pred', 'person_transform_world', 
        # 'person_transform_cam', 'person2cam', 'smpl_orient_world_res', 'root_trans_world_res', 'traj_local_xy', 'traj_local_dxy', 
        # 'traj_local_heading', 'traj_local_dheading', 'traj_local_z', 'traj_local_rot', 'traj_local', 'smpl_orient_cam_in_world', 
        # 'root_trans_cam_in_world', 'kp_2d_pred'])
    ...
    def init_opt(self, data, opt_variables, opt_lr):
        ...
        # print("from global_recon_model.py: opt_variables:",opt_variables) #['cam', 'local_xy', 'local_heading', 'world_dheading', 'local_rot']
        # print("from global_recon_model.py: param_list:",len(param_list)) #6
        print("from global_recon_model.py: param_list:",param_list) #6
        for param in param_list:
