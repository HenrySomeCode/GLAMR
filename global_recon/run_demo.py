...
# print("from run_demo.py: ",pose_est_file) #out/glamr_dynamic/dance/pose_est/pose.pkl
...
if cached and osp.exists(out_file):
    ...
    # print("if: from run_demo.py: out_dict.keys(): ",out_dict.keys())
    
    # print("if: from run_demo.py: out_dict['cam_trans']: ",out_dict['cam_trans'])
    # import pickle
    # with open("/mnt/datadrive/annh/GLAMR/global_recon/cam_trans.pkl","wb") as f:
    #     pickle.dump(out_dict['cam_trans'], f)

    # print("if: from run_demo.py: out_dict['person_data']: ",out_dict['person_data'][0].keys())
    
    # dict_keys(['visible', 'visible_orig', 'fr_start', 'fr_end', 'exist_frames', 'exist_len', 'max_len', 
    # 'frames', 'vis_frames', 'invis_frames', 'frame2ind', 'scale', 'smpl_pose', 'smpl_beta', 'smpl_orient_cam', 
    # 'root_trans_cam', 'kp_2d', 'kp_2d_score', 'kp_2d_aligned', 'cam_K', 'root_trans_world', 
    # 'smpl_orient_world', 'root_trans_world_base', 'smpl_orient_world_base', 'smpl_pose_nofill', 
    # 'infilled', 'traj_predicted', 'traj_local_pred', 'person_transform_world', 'person_transform_cam', 
    # 'person2cam', 'smpl_orient_world_res', 'root_trans_world_res', 'traj_local_xy', 'traj_local_dxy', 
    # 'traj_local_heading', 'traj_local_dheading', 'traj_local_z', 'traj_local_rot', 'traj_local', 
    # 'smpl_orient_cam_in_world', 'root_trans_cam_in_world', 'kp_2d_pred', 'world_dheading'])

    # print("if: from run_demo.py: out_dict['person_data']: ",out_dict['person_data'][0]['person_transform_cam']) # 4
    # print("if: from run_demo.py: out_dict['person_data'][0]['person_transform_world']: ",\
    #     out_dict['person_data'][0]['person_transform_world']) # 4
    
    # print("if: from run_demo.py: out_dict['person_data'][0]['root_trans_cam']: ",\
    #     out_dict['person_data'][0]['root_trans_cam']) # 4
    
    # print("if: from run_demo.py: out_dict['person_data'][0]['root_trans_cam']: ",\
    #     out_dict['person_data'][0]['root_trans_cam']) # 4

    print("if: from run_demo.py: out_dict.: ",out_dict.keys()) 

    import pickle
    with open("/mnt/datadrive/annh/GLAMR/global_recon/root_trans_cam.pkl","wb") as f:
        pickle.dump(out_dict['person_data'][0]['root_trans_cam'], f)
    

else:
    est_dict = pickle.load(open(pose_est_file, 'rb'))
    print("else: from run_demo.py: est_dict", est_dict[0].keys())
    in_dict = {'est': est_dict, 'gt': dict(), 'gt_meta': dict(), 'seq_name': seq_name}
    # global optimization
    out_dict = grecon_model.optimize(in_dict)
    print("else: from run_demo.py: out_dict.keys(): ",out_dict.keys())
    #dict_keys(['seq_name', 'person_data', 'seq_len', 'fr_num_persons', 'cam_pose', 
    # 'cam_pose_inv', 'cam_inv_rot_residual', 'cam_inv_trans_residual', 'rel_transform_cam', 
    # 'gt', 'gt_meta', 'meta', 'pose_infer_cam_pose_inv', 'cam_rot_6d', 'cam_trans'])
    
    pickle.dump(out_dict, open(out_file, 'wb'))

    # print("else: from run_demo.py: out_dict['cam_trans']: ",out_dict['cam_trans'])
    # print("else: from run_demo.py: out_dict['person_data']: ",out_dict['person_data'])
    with open("/mnt/datadrive/annh/GLAMR/global_recon/cam_trans.pkl","wb") as f:
        pickle.dump(out_dict['cam_trans'], f)
    ...
    with open("/mnt/datadrive/annh/GLAMR/global_recon/root_trans_world.pkl","wb") as f:
        pickle.dump(out_dict['person_data'][0]['root_trans_world'], f)
